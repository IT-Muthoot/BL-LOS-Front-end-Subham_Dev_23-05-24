...
@Component({
  selector: 'app-wilful-defaulter-blacklist-browse',
  standalone: true,
  ...
  templateUrl: './wilful-defaulter-blacklist-browse.component.html',
  styleUrl: './wilful-defaulter-blacklist-browse.component.scss'
})
export class WilfulDefaulterBlacklistBrowseComponent {
  ...
  async getData(p:any) {
    ...
    let res:any = await this._cs.getCommonFunction('GetAllWilfulDefaulter', payload);
    if(res.statusCode == 200){
  ...
  async filter(){
    var payload = {
      ...
      searchCriteria: this.filterForm.value.keyFields?parseInt(this.filterForm.value.keyFields):0,
      keyField: this.filterForm.value.searchCriteria?parseInt(this.filterForm.value.searchCriteria):0,
      source: this.filterForm.value.source?this.filterForm.value.source:0,
      lusr:'MF58957',
      pageNo: 1,
      pageSize: this.itemsPerPage
    }
    let res:any = await this.cs.getCommonFunction('FilterdWilfulDefaulter',payload);
    if(res.statusCode == 200){
      this.browseList = res.data;
      this.totalRecords = res.totalRecordCount.recordCount;
    } else {
  ...
  async onStatusChange(selectedId:any, status:any) {
    ...
    dialogRef.afterClosed().subscribe(async result => {
      if (result === 'confirm') {
        ...
        let res:any = await this._cs.getCommonFunction('StatusUpdateWilfulDefaulter', payLoad);
        if(res.statusCode == 200){

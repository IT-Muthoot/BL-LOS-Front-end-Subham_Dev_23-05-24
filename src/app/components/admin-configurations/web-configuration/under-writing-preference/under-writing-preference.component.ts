...
@Component({
  selector: 'app-under-writing-preference',
  standalone: true,
  ...
  templateUrl: './under-writing-preference.component.html',
  styleUrl: './under-writing-preference.component.scss'
})
export class UnderWritingPreferenceComponent {
  totalRecords: any;
  browseList:any = [];
  ...
  async getData(p:any) {
    ...
    let res:any = await this.cs.getCommonFunction('UnderWritingPreferenceGetAll' ,payload);
    if(res.statusCode == 200){
  ...
  async filter(){
    ...
    let res:any = await this.cs.getCommonFunction('UnderWritingPreferenceGetByFilter', payload);
    if(res.statusCode == 200){
  ...
  async onStatusChange(selectedId:any, status:any) {
    ...
    dialogRef.afterClosed().subscribe(async result => {
      if (result === 'confirm') {
        ...
        let res:any = await this.cc.commonAPICall(payLoad, 'UnderWritingPreferenceUpdateStatus');
        if(res.statusCode == 200){

...
@Component({
  selector: 'app-sla-defination-list',
  standalone: true,
  ...
  templateUrl: './sla-defination-list.component.html',
  styleUrl: './sla-defination-list.component.scss'
})
export class SlaDefinationListComponent {
  totalRecords: any;
  browseList:any = [];
  ...
  async getData(p:any) {
    ...
    let res:any = await this.cs.getCommonFunction('GetAllSLAdefinationConfigurationMaster', payload);
    if(res.statusCode == 200){
  ...
  async filter(){
    ...
    let res:any = await this.cs.getCommonFunction('FilterSLAdefinationConfigurationMaster',payload);
    if(res.statusCode == 200){
      this.browseList = res.dataTable;
  ...
  async onStatusChange(selectedId:any, status:any) {
    ...
    dialogRef.afterClosed().subscribe(async result => {
      if (result === 'confirm') {
        ...
        let res:any = await this.cs.getCommonFunction( 'UpdateStatusSLAdefinationConfigurationMaster', payLoad);
        if(res.statusCode == 200){

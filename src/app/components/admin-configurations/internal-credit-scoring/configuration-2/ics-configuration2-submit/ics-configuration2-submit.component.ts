...
@Component({
  selector: 'app-ics-configuration2-submit',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, MiscellaneousDropDownPipe],
  templateUrl: './ics-configuration2-submit.component.html',
  styleUrl: './ics-configuration2-submit.component.scss'
})
export class IcsConfiguration2SubmitComponent {
  ...
  async finalSubmit(){
    ...
    if(this.finalSubmitted && this.parentForm.invalid  && (this.tempForm.invalid || this.tempData.length == 0)){
      ...
      if(this.tempData.length == 0){
        ...
        let res:any = await this._cs.getCommonFunction('InsertInternalCreditScoringTwo', payload);
        if(res.statusCode == 200){

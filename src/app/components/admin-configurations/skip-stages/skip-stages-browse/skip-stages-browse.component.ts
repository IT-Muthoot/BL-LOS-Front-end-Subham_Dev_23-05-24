...
@Component({
  selector: 'app-skip-stages-browse',
  standalone: true,
  ...
  templateUrl: './skip-stages-browse.component.html',
  styleUrl: './skip-stages-browse.component.scss'
})
export class SkipStagesBrowseComponent {
  totalRecords: any;
  browseList:any = [];
  ...
  async getData(p:any) {
    ...
    let res:any = await this.cs.getCommonFunction('GetAllSkipsStageConfigurationHeader' ,payload);
    if(res.statusCode == 200){
  ...
  async filter(){
    ...
    let res:any = await this.cs.getCommonFunction('FilterSkipsStageConfigurationHeader', payload);
    if(res.statusCode == 200){
  ...
  async onStatusChange(selectedId:any, status:any) {
    ...
    dialogRef.afterClosed().subscribe(async result => {
      if (result === 'confirm') {
        ...
        let res:any = await this.cc.commonAPICall(payLoad, 'UpdateStatusSkipsStageConfigurationHeader');
        if(res.statusCode == 200){

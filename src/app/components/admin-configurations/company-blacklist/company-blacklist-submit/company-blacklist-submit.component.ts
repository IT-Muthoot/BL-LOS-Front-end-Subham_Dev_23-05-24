...
@Component({
  selector: 'app-company-blacklist-submit',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, MiscellaneousDropDownPipe,RouterLink],
  templateUrl: './company-blacklist-submit.component.html',
  styleUrl: './company-blacklist-submit.component.scss'
})
export class CompanyBlacklistSubmitComponent {
  parentForm:any
  tempForm: any;
  ...
  ngOnInit(){
    ...
    this.tempForm = this._fb.group({
      cinOrGst: ['', [Validators.required, Validators.minLength(3), Validators.maxLength(50)]],
      companyName: ['', [Validators.required, , Validators.minLength(3), Validators.maxLength(50)]],
      din: ['', [Validators.required, , Validators.minLength(3), Validators.maxLength(50)]],
  ...
  editTempGrid(index:any){
    ...
    this.tempForm = this._fb.group({
      cinOrGst: [this.tempData[index].cinOrGst, [Validators.required, Validators.minLength(3), Validators.maxLength(50)]],
      companyName: [this.tempData[index].companyName, [Validators.required, , Validators.minLength(3), Validators.maxLength(50)]],
      din: [this.tempData[index].din, [Validators.required, , Validators.minLength(3), Validators.maxLength(50)]],
    ...
    if(this.finalSubmitted && this.parentForm.invalid  && (this.tempForm.invalid || this.tempData.length == 0)){
      ...
      if(this.tempData.length == 0){
        ...
        var payload = {
          ...
          lusr: "john_doe",
          status: 1,
          companyDetails: this.tempData
        }
        let res:any = await this._cs.getCommonFunction('InsertCompanyBlacklist', payload);
        if(res.statusCode == 200){
